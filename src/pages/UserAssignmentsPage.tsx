...
import {
  assignArchitectureUserToDiets, assignUserToAgency, assignUserToDiet, deactivateUserAssignment, getApprovedAssignableUsers,
...
export function UserAssignmentsPage() {
  ...
  const [mode, setMode] = useState<"assign" | "view" | "deactivate" | null>(null), [targetId, setTargetId] = useState(""), [targetDietIds, setTargetDietIds] = useState<string[]>([]), [remarks, setRemarks] = useState("");
  ...
  async function open(user: AppUser, nextMode: typeof mode) {
    setSelected(user); setMode(nextMode); setRemarks(user.assignmentRemarks ?? ""); setTargetId(user.primaryDietId ?? user.primaryAgencyId ?? ""); setTargetDietIds(user.assignedDietIds ?? []); setError("");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      else if (selected.organisationRole === "architecture_department") await assignArchitectureUserToDiets(selected, diets.filter(d => targetDietIds.includes(d.id)), remarks);
  ...
  return <>
    <header className="page-header"><span className="eyebrow">Administration</span><h1>Approved User Assignments</h1><p>Assign approved DIET, PWD, RDP and Architecture users to the records they are authorised to manage.</p></header>
    ...
    <div className="stat-grid assignment-stats">
      ...
      {(["diet", "pwd", "rdp", "architecture_department"] as const).map(r => <section className="stat-card" key={r}><span>{r === "architecture_department" ? "Architecture users awaiting assignment" : `${r.toUpperCase()} users awaiting assignment`}</span><strong>{unassigned.filter(u => u.organisationRole === r).length}</strong></section>)}
    </div>
