...
export function WorkPackagesPage() {
  ...
  const selectedScope = historicalMode ? undefined : scopes.find((item) => item.id === form.scopeId);
  ...
  return <>
    ...
    {formOpen && canCreate && <section className="section-band"><div className="section-title"><h2>{editing ? "Edit Draft Work Package" : "Create Work Package"}</h2><button className="secondary-button" onClick={() => setFormOpen(false)}><X size={16}/>Close</button></div><form className="agency-form" onSubmit={save}>
      <label>DIET *<select value={form.dietId} disabled={Boolean(editing)} onChange={(e) => setForm({ ...form, dietId: e.target.value, assignmentId: "", scopeId: "", activityAmounts: {} })}><option value="">Select DIET</option>{diets.filter((item) => assignments.some((a) => a.status === "active" && a.dietId === item.id)).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
