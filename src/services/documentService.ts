...
if(metadata.entityType==="scope_of_work" && !await getScopeOfWork(metadata.entityId))throw new Error("Scope not found."); if(file.type!=="application/pdf")throw new Error("Only PDF attachments are permitted.");if(file.size<=0||file.size>MAX_PDF_SIZE)throw new Error("PDF attachments must be no larger than 15 MB.");
