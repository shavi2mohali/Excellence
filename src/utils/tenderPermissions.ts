...
export function canTransitionTenderStatus(current: Tender["status"], next: Tender["status"], userRole?: string) {
  const transitions: Partial<Record<Tender["status"], Tender["status"][]>> = userRole === "scert_admin"
    ? { submitted_for_review: ["revision_required", "approved_for_publication"], approved_for_publication: ["published"] }
    : userRole === "agency_user"
      ? { draft: ["submitted_for_review"], revision_required: ["submitted_for_review"], approved_for_publication: ["published"] }
      : {};
...
export const canPublishTender = (profile: AppUser | null, tender: Tender) => tender.status === "approved_for_publication" && (role(profile) === "scert_admin" || (role(profile) === "agency_user" && (profile?.assignedAgencyIds || []).includes(tender.executingAgencyId)));
